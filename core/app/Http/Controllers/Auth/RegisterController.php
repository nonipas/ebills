    ...
    protected function create(array $data)
    {
        ...
        if($user->ref_by){
            ...
            if($gnl->upline_ref_sign_up != 0){
                ...
                $transaction->user_id = $getUpline->id;
                $transaction->amount = getAmount($gnl->upline_ref_sign_up);
                $transaction->post_balance = getAmount($getUpline->balance);
                $transaction->charge = 00;
                $transaction->trx_type = '+';
                $transaction->details = 'Sign up bonus for share referral link';
